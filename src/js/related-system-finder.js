...
const getRelatedSystem = function (cardNum) {
    ...
    if (firstTwo == '54' || firstTwo == '36' || isInRange(parseInt(firstThree), {min: 300, max: 305})) {
    ...
    if (isInRange(parseInt(firstTwo), {min: 51, max: 55}) || isInRange(parseInt(firstSix), {min: 222100, max: 272099})) {
        return {tag: 'mastercard', name: 'MasterCard'};
    }

    if (firstFour == '6011' || firstTwo == '65' || isInRange(parseInt(firstSix), {min: 622126, max: 622925}) || isInRange(parseInt(firstThree), {min: 644, max: 649})) {
